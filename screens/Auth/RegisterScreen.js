...
export default function RegisterScreen({ navigation }) {
  ...
  const sendCode = async () => {
    if (!phone || !phone.startsWith('+')) {
      Alert.alert('Invalid phone format', 'Please use +91xxxxxxxxxx format.');
      return;
    }
    try {
      const phoneProvider = new firebase.auth.PhoneAuthProvider();
      const id = await phoneProvider.verifyPhoneNumber(phone);
      setVerificationId(id);
      Alert.alert('Verification code sent');
    } catch (e) {
      Alert.alert('Phone Error', e.message);
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (image) {
        const response = await fetch(image.uri);
        const blob = await response.blob();
        const ref = storage().ref().child(`profiles/${user.uid}.jpg`);
        await ref.put(blob);
        photoURL = await ref.getDownloadURL();
      }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TextInput placeholder="Designation" value={designation} onChangeText={setDesignation} style={styles.input} />
      ...
      {image && (
        <Image source={{ uri: image.uri }} style={{ width: 80, height: 80, marginBottom: 12, borderRadius: 40 }} />
      )}
